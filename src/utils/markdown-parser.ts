...
export class MarkdownParser {
  ...
  private parseListGroup(lines: string[], startIndex: number): { html: string, nextIndex: number } {
    const listItems: Array<{content: string, level: number, type: 'ul' | 'ol'}> = [];
    let i = startIndex;
  ...
  private buildNestedList(items: Array<{content: string, level: number, type: 'ul' | 'ol'}>): string {
    ...
    while (i < items.length) {
      ...
      while (i < items.length && items[i].level === currentItem.level) {
        ...
        const nestedItems: Array<{content: string, level: number, type: 'ul' | 'ol'}> = [];
        let j = i + 1;
