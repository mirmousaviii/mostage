...
export class BasicTemplateStrategy implements TemplateStrategy {
  async create(projectPath: string, options: ProjectOptions): Promise<void> {
    ...
    if (options.createConfigFile) {
      await this.copyConfigFile(projectPath, templatePath, options);
    }

    // Copy index.html
    await this.copyIndexHtml(projectPath, templatePath);
  ...
  private async copyContentFile(
    ...
    const contentSource = path.join(templatePath, "content.md");
    const contentDest = path.join(
      projectPath,
      options.contentPath || "./content.md"
    );

    if (await fs.pathExists(contentSource)) {
      await fs.copy(contentSource, contentDest);
    }
  }

  private async copyConfigFile(
    projectPath: string,
    templatePath: string,
    options: ProjectOptions
  ): Promise<void> {
    const configSource = path.join(templatePath, "config.json");
  ...
  private async copyIndexHtml(
    projectPath: string,
    templatePath: string
  ): Promise<void> {
    const indexSource = path.join(templatePath, "index.html");
    const indexDest = path.join(projectPath, "index.html");
...
export class DemoTemplateStrategy implements TemplateStrategy {
  ...
  private async createConfigFile(
    ...
    if (options.centerVertical !== undefined && config.centerContent) {
      config.centerContent.vertical = options.centerVertical;
    }
    if (options.centerHorizontal !== undefined && config.centerContent) {
      config.centerContent.horizontal = options.centerHorizontal;
    }
...
export class CustomTemplateStrategy implements TemplateStrategy {
  async create(projectPath: string, options: ProjectOptions): Promise<void> {
    // Ensure project directory exists
    await fs.ensureDir(projectPath);
  ...
  private async createConfigFile(
    ...
    await fs.writeFile(path.join(projectPath, "config.json"), configJson);
    console.log(chalk.gray("   - config.json"));
  }

  private logSuccess(options: ProjectOptions): void {
    console.log(chalk.green("✅ Custom template created successfully!"));
    console.log(chalk.gray("   - index.html (main HTML file)"));
    if (options.createConfigFile) {
      const configPath = options.configPath || "./config.json";
      console.log(chalk.gray(`   - ${configPath} (configuration)`));
