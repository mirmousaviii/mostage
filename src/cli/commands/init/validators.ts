import { InitOptions, InitError } from "../init";
...
export class ProjectValidator {
  static validateOptions(options: InitOptions): void {
    if (
      options.template &&
      !Object.values(TEMPLATES).includes(options.template as any)
    ) {
      throw new InitError(
        `Invalid template: ${options.template}. Available templates: ${Object.values(TEMPLATES).join(", ")}`,
        "INVALID_TEMPLATE"
      );
    }
